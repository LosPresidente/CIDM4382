...
const IsInARTCC = (client) => {
    //Departure airport- planned_depairport
    //Destination airport- planned_destairport
    if(planned_depairport == "KMSP"){
        console.log("Depart airport found");
    }

    if(planned_desairport == "KMSP"){
        console.log("Destination aiport found airport found");
    }
...
const writeClientModelListToPersist = (client_list) => {
    ...
    //put here the url from my DB
    const uri = process.env.MONGODB_ATLAS_URL;
    
    // what to fill in the username and password can be found in .env file
    //const uri = `mongodb+srv://<username>:<password>@cluster0-jcxfe.mongodb.net/vatsim?retryWrites=true&w=majority`;
    mongoose.connect(uri, {useNewUrlParser: true, useUnifiedTopology: true});
    
    //take this info from my cluster (Client)
